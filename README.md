...
**Status:** **Phase 2 closed (2026-05-25).** Per-tenant data plane is live: every operational entity (customers, contacts, sites, staff, licences, schedules, prestarts, toolbox talks, tenders, service jobs, quotes, intake batches) lives exclusively on a per-tenant Supabase project (`eq-canonical-internal`, `sks-canonical`). Shared `eq-canonical` holds only the control plane (`shell_control` — tenants, users, entitlements, encrypted `tenant_routing`) plus auth/storage. Security audit Blocks 1 + 2 shipped same day (CSP + headers, fail-loud secrets, fail-closed rate limit, bcrypt 12, uniform auth-error responses, per-app tenant scope on `canonical-api`); standards captured in [SECURITY-PATTERNS.md](SECURITY-PATTERNS.md). The **Intake module** (`/core/intake`) is live behind login, routing every write through the per-tenant `intake-commit` orchestrator. The earlier `src/modules/tender-pipeline/` scaffolding and the `jobs` module have both been removed (2026-05-23 and Phase 2.B.7 respectively). See [docs/ARCHITECTURE-V2.md](docs/ARCHITECTURE-V2.md) for the full closed-out phase plan and [docs/FIELD-UNIFICATION-PLAN.md](docs/FIELD-UNIFICATION-PLAN.md) for the next planned arc (collapse three Field deploys into one shell-mounted module).
