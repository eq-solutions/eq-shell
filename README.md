...
**Status:** Phase 1.G shipped (2026-05-23). Phase 1.F (Unified Identity) shipped 2026-05-20. **Active sprint: Phase 2 — per-tenant data plane migration.** Splits the data plane (`app_data`) into per-tenant Supabase projects while keeping the control plane (`shell_control`) shared. Removes the structural risk of cross-tenant data colocation and unlocks the canonical-layer-as-platform positioning. Sequencing in [docs/ARCHITECTURE-V2.md](docs/ARCHITECTURE-V2.md). The **Intake module** (`/core/intake`) is live behind login, running the `@eq/*` engine end-to-end against the canonical data plane. Earlier `src/modules/tender-pipeline/` scaffolding is stale exploration and not on the roadmap.
...
|---|---|---|
| Supabase project | `eq-canonical` (id: `jvknxcmbtrfnxfrwfimn`, region `ap-southeast-2`) | **Control plane** — holds shell control tables (`tenants`, `users`, `module_entitlements`, `tenant_routing`). Until Phase 2.B.4/5 cutover completes, also holds the shared `app_data` schema for both EQ + SKS rows (RLS-isolated). After cutover, `app_data` is dropped here and lives only in the per-tenant projects below. |
| Supabase project | `eq-canonical-internal` (id: `zaapmfdkgedqupfjtchl`, region `ap-southeast-2`) | **EQ tenant data plane** (Phase 2.B). Schema baseline applied; awaiting data migration via `scripts/sync-tenant-data.mjs --slug=core`. Routing row in `provisioning` state. |
| Supabase project | `sks-canonical` (id: `ehowgjardagevnrluult`, region `ap-southeast-2`) | **SKS tenant data plane** (Phase 2.B). Schema baseline applied; data migration requires a scheduled maintenance window with SKS users. Routing row in `provisioning` state. |
...
| 2.B | **Per-tenant data plane migration** — split `app_data` out of the shared `eq-canonical` into one Supabase project per tenant; introduce `shell_control.tenant_routing` + encrypted service-role storage; build `canonical-api` as the service-to-service read/write layer; build the schema migration runner; migrate EQ internal, then SKS. **2026-05-24: Phases 2.B.1–2.B.3 complete** — `tenant_routing` table live, encryption helpers + routing helper shipped, `canonical-api` + `tenant-routing-health` functions deployed, two tenant Supabase projects (`eq-canonical-internal`, `sks-canonical`) provisioned with baseline schema, routing rows in `provisioning` state. Phase 2.B.4/5 (data sync + cutover) is the next step — see cutover runbook in [docs/ARCHITECTURE-V2.md](docs/ARCHITECTURE-V2.md). | Active sprint |
| 3+ | Replace each EQ Field surface (roster, schedule, leave, tenders, audits, prestarts, toolbox talks) with a shell module backed by the per-tenant canonical data plane. Each surface goes live in the shell, then its EQ Field equivalent gets retired. Sequencing driven by Royce's operational priorities at SKS NSW. | Long-term |
