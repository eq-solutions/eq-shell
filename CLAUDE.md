...
| `netlify/functions/token-exchange.ts` (Supabase JWT — HS256 `SUPABASE_JWT_SECRET`, 60s TTL; `aud='field'` + `aud='service'`) | eq-solves-field iframe handoff — Field `verify-pin.js` (action `verify-shell-token`) verifies the JWT; eq-solves-service `/shell` → `/api/shell-auth`. **Replaced the HMAC `mint-iframe-token.ts` (removed in Phase 3/4 HMAC retirement). Service is still the `aud='service'` branch — no separate minter.** |
