...
// Full patch covers all editable line fields — both core and invoice
...
export default withSentry(async (req: Request, _ctx: Context): Promise<Response> => {
  ...
  if (req.method === 'POST') {
    ...
    // Derive actor display name for audit log entries
    ...
    if (body.line_id !== undefined) {
      ...
      const parts: string[] = [];
      if (safe.description)              parts.push(`desc updated`);
      if (safe.invoice_number)           parts.push(`invoice ${safe.invoice_number}`);
      if (safe.invoiced_amount != null)  parts.push(`invoiced $${safe.invoiced_amount}`);
      if (safe.price_ex_gst != null)     parts.push(`value $${safe.price_ex_gst}`);
      await logEvent(db, job_id, session.user_id, actor, 'update_line',
        `Line updated: ${parts.join(', ') || 'fields updated'}`);
    ...
    if (body.line !== undefined) {
      ...
      await logEvent(db, job_id, session.user_id, actor, 'add_line',
        `PO line added: ${String(raw.description)}`);
      return json(200, { ok: true, line: data });
    }
  ...
  // Single job with PO lines + last 50 events
  if (jobId) {
  ...
  // All jobs with aggregated PO line totals (incl. materials cost)
  ...
  for (const row of (agg ?? []) as LineRow[]) {
    if (!totals[row.job_id]) {
      totals[row.job_id] = { total_value: 0, total_invoiced: 0, total_hours: 0, total_materials: 0, line_count: 0 };
    }
