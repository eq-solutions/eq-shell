...
| `sks_gm_briefing_reshape_expand.sql` | ehowg | 🔒🔒 | ✅ **APPLIED** 2026-06-01 (`sks_gm_briefing_reshape_expand`) | PHASE 1: added `tenant_id` (nullable) + backfill `7dee117c` + per-tenant index. |
| `sks_gm_briefing_reshape_contract.sql` | ehowg | 🔒🔒 | ✅ **APPLIED** 2026-06-01 (`sks_gm_briefing_reshape_contract`) | PHASE 2: enforced NOT NULL + swapped period unique key to `(tenant_id, period_code)`. Code change (PRs #107 + #108) deployed first. |

| `sks_intake_vestige_tables_drop.sql` | ehowg | 🔒 | ✅ **APPLIED** 2026-06-01 (`drop_intake_vestige_tables`) | Dropped 4 leftover `eq_intake_*` tables. All confirmed empty / rolled-back smoke data at time of drop. |
