...
## Applied state (queried 2026-06-02)
...
| `0012_dashboard_counts_v2.sql` | Dashboard count improvements | ✅ 2026-05-28 | ✅ 2026-06-02 |
| `0013_write_canonical_event_rpc.sql` | Write-canonical-event RPC | ✅ 2026-05-28 | ✅ 2026-06-02 |
...
| `0018_dashboard_counts_asset.sql` | Asset count tile on dashboard | ✅ 2026-06-02 | ✅ 2026-05-29 |
| `0019_dashboard_asset_service_due.sql` | Service-due asset card on dashboard | ✅ 2026-06-02 | ✅ 2026-05-29 |
| `0020_service_cmms.sql` | CMMS core schema (work orders, maintenance records) | ✅ 2026-06-02 (no-op — tables existed) | ✅ 2026-05-30 |
| `0021_service_ppm_rpcs.sql` | PPM (Planned Preventive Maintenance) RPCs | ✅ 2026-05-30 | ✅ 2026-05-30 |
| `0022_canonical_write_rpcs.sql` | Canonical write RPCs | ✅ 2026-06-02 | ✅ 2026-05-30 |
| `0023_intake_infra.sql` | Intake infrastructure tables | ✅ 2026-05-30 | ✅ 2026-06-02 |
| `0024_gm_reports.sql` | GM Reports — `gm_report_periods` + `gm_report_jobs` (idempotent) | ✅ 2026-06-02 (no-op tables; RLS fixed with `::uuid` cast — see ⚠️ below) | ✅ 2026-06-02 |
| `0025_briefing.sql` | AI briefing — `briefing_cache` + `briefing_actions` with `tenant_id NOT NULL` | ✅ 2026-06-02 (no-op tables; RLS fixed with `::uuid` cast — see ⚠️ below) | ✅ 2026-06-02 |
| `0026_briefing_cache_and_actions.sql` | Same tables, older nullable shape + `ADD COLUMN IF NOT EXISTS tenant_id` safety net | ✅ 2026-06-02 (no-op — column existed) | ✅ 2026-06-02 (no-op — column created by 0025) |
| `0027_drop_intake_vestiges.sql` | Drop orphaned `eq_intake_*` tables | ✅ 2026-06-02 (no-op — already dropped 2026-06-01) | ✅ 2026-06-02 |
| `0028_contact_customer_links.sql` | Contact ↔ customer links table | ✅ 2026-06-02 (was genuinely missing) | ✅ 2026-05-30 |
| `0029_safety_rpcs.sql` | Prestart/toolbox submit + approve RPCs with JWT tenant guard | ✅ 2026-06-02 | ✅ 2026-06-02 |
...
**All migrations 0001–0029 are now applied on both tenants (2026-06-02).** No pending items.

Next migration file: `0030_*.sql`

## Schema type discrepancy (ehow vs zaap)

During 2026-06-02 catch-up, the following `tenant_id` columns were found as **`uuid`** on ehow (SKS) rather than `text` as declared in the canonical migrations. This is because the reshape migrations (`sks_gm_briefing_reshape_*`) added these columns via `ALTER TABLE ... ADD COLUMN tenant_id uuid`, not `text`.

| Table | ehow type | zaap type | Migration policy fix applied |
|-------|-----------|-----------|------------------------------|
| `gm_report_periods` | `uuid` | `text` | `::uuid` cast in RLS policy |
| `briefing_cache` | `uuid` | `text` | `::uuid` cast in RLS policy |
| `briefing_actions` | `uuid` | `text` | `::uuid` cast in RLS policy |

**Future tenant provisioning:** when running the migration runner against a fresh tenant, these tables will be created with `tenant_id text` (as the migration files declare). This diverges from ehow's live schema. Decision needed: normalise both to `uuid` (preferred — matches Supabase's native `auth.uid()` type) or normalise both to `text`. Track as a separate migration.
