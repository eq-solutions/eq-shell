-- Migration 0117
-- HOTFIX: 0116 redefined eq_list_quotes with a single-arrow JWT extraction
--   ... -> 'app_metadata' -> 'tenant_id')::uuid
-- The `->` returns jsonb, so `::uuid` raises "cannot cast type jsonb to uuid"
-- and every eq_list_quotes call fails — breaking the pipeline list AND the
-- post-save reload (surfacing as a "save failed" error). Must be `->>` (text).
-- Forward-only fix per SCHEMA-GOVERNANCE.md (0116 is left as-is; this supersedes
-- it). No ledger self-insert — the runner records the row on apply.
